...
local chatglobal = {"VIP ADV", "PREMIUM", "VIP", "FOREVER", "Ïèëîò ãðàæäàíñêîé àâèàöèè","Ïîæàðíûé","Ìåõàíèê","Äàëüíîáîéùèê","Òàêñèñò","Èíêàññàòîð","Ìóñîðùèê","Ðàáîòíèê íàëîãîâîé","Ðàçâîç÷èê ìåòàëëîëîìà","Âîäèòåëü àâòîáóñà","Ðàçâîç÷èê ïðîäóêòîâ", "Àäâîêàò", "Âîäèòåëü òðàìâàÿ", "Ìàøèíèñò ýëåêòðîïîåçäà", "Ãëàâíûé ôåðìåð", "Ðóêîâîäèòåëü ãðóç÷èêîâ", "Ðóêîâîäèòåëü çàâîäà", "Ðåìîíòíèê äîðîã", "Ïðîäàâåö õîòäîãîâ", "Ïàðêîâùèê"}
    ...
    httpRequest('http://144.126.154.29:25637/api/loginingame?nick=' .. clientName, nil, function (e,r)
        if r == 200 then
            ...
            if response['login'] then
                ...
                sampAddChatMessage(tagchat .. "Ó âàñ îòñóòñòâóåò äîñòóï â áîòå.", -1)
                thisScript():unload()
            end
        else
    ...
    while true do
        ...
        if isKeyJustPressed(settings.main.key_accept_form) then
			...
			if checktime <= settings.main.time_wait_form and status_formi then
				...
				sampAddChatMessage(tagchat .. 'Âû óñïåøíî {1aff00}ïðèíÿëè{ffffff} ôîðìó íà èãðîêà!', -1)
				printStyledString("~g~~h~Form accepted", 3000, 5)
			else
				status_formi = false
				statusnotf = false
				sampAddChatMessage(tagchat .. 'Íà äàííûé ìîìåíò íåò àêòóàëüíûõ ôîðì äëÿ âûäà÷è íàêàçàíèÿ!', -1)
			end
		end
    end
end
...
function addform(ggggg, formai, nicki, idi, textio)
    ...
    sampAddChatMessage(tagchat .. 'Èãðîê {0080ff}' .. tostring(nicki) .. '[' .. tostring(idi) ..']{ffffff} íàðóøèë ïðàâèëà ñåðâåðà!', -1)
    sampAddChatMessage(tagchat .. 'Íàæìèòå {c92800}' .. vkeys.id_to_name(settings.main.key_accept_form) .. '{ffffff} äëÿ áûñòðîé âûäà÷è ôîðìû: {9f63ff}' .. forma, -1)
